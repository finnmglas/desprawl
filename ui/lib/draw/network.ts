...
import { lanesOf, placing } from "./laying.ts"
import { BUDGET, PAD, ROW, pack, rounded } from "./packing.ts"
import { unitOf, type Layout } from "../../../src/read/layers.ts"
...
export function net(
  ...
  const { boxes: sat, widest, deepest: down } = placing(laid, lanes, width, tall)
  boxes.push(...sat)
  let deepest = down
