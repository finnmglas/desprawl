...
const STATIC = new RegExp(
  // the clause stops at the next import or export, or `export { a }` would swallow the one after it
...
export const breaks = (code: string): number[] => {
  const at = [0]
  for (let i = code.indexOf("\n"); i !== -1; i = code.indexOf("\n", i + 1)) at.push(i + 1)
  return at
}

export const lineAt = (starts: number[], index: number): number => {
  let low = 0
