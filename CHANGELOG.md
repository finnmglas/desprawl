...
- **Tests are counted in whatever language they were written in.** A pytest, `go test` or `cargo test` suite used to read as `files: 0, cases: 0`, which is indistinguishable from a repo with no tests. Counted by reading, exactly as the javascript half always was, and the card says what would run them.
- **A python dependency knows its version, its licence and its size**, off the `.dist-info` in the venv the way npm's are read off `node_modules`, with `uv.lock` and `poetry.lock` pinning what no venv on disk can answer. Advisories are asked for on the Pub and PyPI ecosystems the same way.
...
- **Somebody else's code was counted as this project's.** A repo that commits its `node_modules` read as 437k lines for a fifteen file project, and its cleanup list opened with "fix the import of ./regexp in ace.js". Vendored code is left out of the size, the languages, the tree and the token estimate now, and the masthead says how many files that was. The three copies of the vendored pattern that had drifted apart are one.
- **A pdf was a language.** Formats that are binary by definition are known outright rather than sniffed for a null byte in the first 8KB, which a spec pdf does not have. The masthead also says how much of the line count and the token estimate is code rather than data.
- **A route decorator was read twice**, once as the endpoint it is and once as an outbound request it is not, so a fastapi backend carried hundreds of phantom call sites at the exact lines of its own routes. What served cannot also ask.
- **A document describing another repo's api stood as its server.** In a folder of repos, a frontend holding a codegen snapshot of the backend's openapi spec claimed the backend's endpoints and every edge into them pointed backwards. Code answers, a document describes: where both exist the code is the server, and the api view says how many were folded.
- **Call sites with no readable path were dropped in silence.** A production data layer keeping its paths one hop away in a constants file read as almost no requests at all. They are counted and reported now ("N call sites were written without a path to read"), which is the honest half of a fix whose other half is still to come.
- **Dead code that was provably alive.** A class or function declared where a value goes (`stubGlobal("X", class X {})`, `return function go() {}`) runs where it sits and is never a delete task; a caller through an `export *` barrel is a caller; and a test's own declarations are left out of delete tasks, since deleting them breaks the suite an agent was told to protect.
- **A test suite read as source architecture.** `e2e_tests`, `end_to_end_tests`, `manual_testing` and `load_testing` are tests, `archive` and `documentations` are support: the role patterns knew the javascript conventions and none of the underscore compounds a python repo writes. And a leftover `parent/*` box heavier than the named boxes beside it is opened rather than drawn as the second largest module on the board.
- **A folder said the same word twice when it borrowed one.** `src/sources` read as "Source sources", since `src` spells out to source and the clash that made it borrow never checked whether the word was already there. Plural counts as the same word now, so it is "Sources", and a folder above that says something else is still worth borrowing.
- **Vector art and fixture identifiers outranked real copy** in the repeated-text panel: svg path data, uuids, hashes and base64 are strings no person typed, and a literal only mocks and fixtures repeat is a fixture.
- **Every stage read and scrubbed the same file again.** Four of them wanted the same bytes; they share one bounded, mtime-checked cache now, which is 11% off the api view on one repo and 18% on another, with byte-identical output.
